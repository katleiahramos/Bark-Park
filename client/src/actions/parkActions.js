...
export function findNearByParks(currentLocation) {
  ...
  return dispatch => {
    ...
    return fetch(
      `https://api.foursquare.com/v2/venues/search?ll=${lat},${lng}&query=dog+park&limit=5&client_id=${id}&client_secret=${secret}&v=20181004`
    )
      .then(resp => resp.json())
      .then(results => {
...
export function updatePark(park) {
  ...
  return dispatch => {
    ...
    const body = JSON.stringify({
      park: { name: park.name, address: park.address, count: park.count }
    });
...
export function checkOut(checkIn) {
  return dispatch => {
    dispatch({ type: "BEGIN_PARKS_REQUEST" });
    return fetch(`/api/checkins/${checkIn.id}/checkout`).then(resp => {
